...
def process_video_prune(video_path, max_frames_num, obj_list, query, additional_frames, fps=1, force_sample=False):
    ...
    if len(obj_list) > 0:
        ...
        query_relation = f"{query}\nDoes the image contain any unnatural or impossible details that would not exist in the real world? A. Yes (contains unrealistic elements) B. No (purely real-world scene) Answer with the option's letter directly."
...
def infer(video_path):
    ...
    query = "You have been shown one image."
    ...
    print("is_taken_from_real_world:",bool_value)  # True 或 False
